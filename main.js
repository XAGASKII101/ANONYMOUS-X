...
const removeCmdCommand = require("./commands/removecmd")
...
global.ytch = settings.channelName
...
async function handleMessages(sock, messageUpdate, printLog) {
  try {
    ...
    const ownerCommands = [
      ...
      ".areact", ".autoreact", ".whoami", ".removecmd"
    ]
    ...
    // Command handlers
    switch (true) {
      case userMessage === ".whoami":
        await whoamiCommand(sock, chatId, message)
        break

      case userMessage.startsWith(".removecmd"):
        await removeCmdCommand(sock, chatId, message)
        break

      case userMessage.startsWith(".define") || userMessage.startsWith(".dictionary"):
        await dictionaryCommand(sock, chatId, message)
        break

      case userMessage.startsWith(".add"):
        const numberToAdd = userMessage.split(" ")[1]
        await addCommand(sock, chatId, message, numberToAdd)
        break
      ...
      case userMessage.startsWith(".mode"):
        ...
        try {
          ...
          fs.writeFileSync("./data/messageCount.json", JSON.JSON.stringify(data, null, 2))
      ...
      case userMessage.startsWith(".ttt") || userMessage.startsWith(".tictactoe"):
        const gameText = userMessage.startsWith(".ttt") ? userMessage.slice(4).trim() : userMessage.slice(10).trim()
        await tictactoeCommand(sock, chatId, senderId, gameText)
        break
      ...
      case userMessage.startsWith(".guess"):
        ...
        if (guessedLetter) {
          await guessLetter(sock, chatId, guessedLetter)
        } else {
          await sock.sendMessage(chatId, { text: "Please guess a letter using .guess <letter>" })
        }
        break
      ...
      case userMessage.startsWith(".answer"):
        ...
        if (answer) {
          await answerTrivia(sock, chatId, answer)
        } else {
          await sock.sendMessage(chatId, { text: "Please provide an answer using .answer <answer>" })
        }
        break
      ...
      case userMessage.startsWith(".stupid") ||
        ...
        const quotedMsgStupid = message.message?.extendedTextMessage?.contextInfo?.quotedMessage
        const mentionedJidStupid = message.message?.extendedTextMessage?.contextInfo?.mentionedJid || []
        const stupidArgs = userMessage.split(" ").slice(1)
        await stupidCommand(sock, chatId, quotedMsgStupid, mentionedJidStupid, senderId, stupidArgs)
        break

      case userMessage.startsWith(".truth"):
        await truthCommand(sock, chatId, message)
        break

      case userMessage.startsWith(".dare"):
      ...
      case userMessage === ".vv":
        const viewOnceArgs = userMessage.split(" ").slice(1)
        await viewOnceCommand(sock, chatId, message, viewOnceArgs)
        break
    ...
    if (messageUpdate.messages[0].key.remoteJid) {
      await sock.sendMessage(messageUpdate.messages[0].key.remoteJid, {
